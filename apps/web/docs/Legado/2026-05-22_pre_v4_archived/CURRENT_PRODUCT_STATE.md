...
Nota 2026-05-18: `VISUAL-DNA-03` elevou a camada visual para raster premium. Login, Home, Ranking, pagina publica do local, perfil publico, ligas/torneios e Central de Trabalho agora usam os PNGs premium gerados (`hero-*-premium.png`, `card-*-premium.png`) em vez de depender da primeira base SVG. O shell global recebeu textura sutil de quadra, heros ficaram mais cinematograficos, atalhos ganharam presenca visual com icone/tile/seta curta e cards de descoberta/metricas/objetos ficaram menos administrativos. A regra de negocio, rotas e permissoes foram preservadas; a mudanca foi restrita a linguagem visual, composicao e assets.

Nota 2026-05-18: iniciado o refinamento visual premium do ATP com foco em imagem, textura esportiva e componentes reutilizaveis. Foram criados `VisualHeroCard`, `ShortcutCard`, `MetricCard` e `VisualBadge`, alem de assets SVG locais para hero de quadra, quadra noturna, perfil/quadra, card de estadio e icones de reservar/jogar/aulas/ligas. Login, Home do jogador, Ranking e Central de Trabalho receberam primeira camada visual mais esportiva, com tokens de radius/sombra/surface ajustados. A documentacao da rodada esta em `VISUAL_REFINEMENT_SPRINT_2026_05_18.md` e os prompts/dimensoes para imagens raster futuras estao em `VISUAL_ASSET_PROMPTS.md`. `npm.cmd run lint` e `npm.cmd run build` passaram.

Nota 2026-05-18: `VISUAL-DNA-02` concluido. A linguagem visual premium foi expandida para Locais, Perfil publico, Liga e Torneio. Foram criados `visual-club-hero.svg`, `visual-lesson-hero.svg` e `visual-match-hero.svg`; a pagina publica do local usa hero contextual com imagem quando nao ha cover real; action rail, cards de reserva/aulas/jogos/planos, perfil publico, scouting privado, `CompetitionHeader`, `CompetitionTabs` e heros de liga/torneio receberam camadas de imagem/textura, radius e sombras mais esportivas. Nenhuma regra de negocio, rota profunda ou permissao foi alterada. `npm.cmd run lint` e `npm.cmd run build` passaram.
