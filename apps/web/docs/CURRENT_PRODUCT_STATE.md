...
- `SCREEN-LEAGUE-05` recebeu entrega parcial: Chat/avisos da liga foi alinhado ao padrao visual do chat de torneio, com aviso fixado destacado, feed simples e ferramentas administrativas restritas ao organizador.
- Validacao de `SCREEN-LEAGUE-05`: `git diff --check`, `npm.cmd run lint` e `npm.cmd run build` passaram; falta validar screenshots autenticados desktop/mobile e mensagens muito longas.
- `SCREEN-GESTAO-01` recebeu entrega parcial: central `/gestao` prioriza fila do dia e locais sob gestao antes de metricas e setup; implantacao e base incompleta agora ficam recolhidas para nao contaminar a rotina diaria.
- Validacao de `SCREEN-GESTAO-01`: `git diff --check`, `npm.cmd run lint` e `npm.cmd run build` passaram; falta validar screenshots autenticados desktop/mobile, especialmente 390px com muitos locais.
- `SCREEN-GESTAO-AGENDA-01` recebeu entrega parcial: Agenda prioriza pendencias acionaveis na primeira dobra, ordena reservas pendentes antes da lista do dia e evita renderizar centenas de reservas/espera sem filtro usando resumo visivel + `Ver todas`.
- Validacao de `SCREEN-GESTAO-AGENDA-01`: `git diff --check`, `npm.cmd run lint` e `npm.cmd run build` passaram; falta validar screenshots autenticados desktop/mobile e exportacao real de agenda por quadra em PNG.
