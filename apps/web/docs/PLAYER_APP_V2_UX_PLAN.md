...
- `QA-DESIGN-01` removeu contadores zerados dos tiles de intencao; se nao ha dado positivo, a intencao continua clara sem parecer dashboard vazio;
...
- implementado em `PLAYER-UX-05`;
- `/locais?intencao=jogos` usa busca rapida, filtros em disclosure e rows de chamada;
- `Criar chamada` fica como CTA contextual e abre formulario curto somente quando o jogador quer publicar;
- `Quero jogar`/`Fechar chamada` sao as acoes primarias nas rows;
- comentarios e interesse foram mantidos em `Detalhes`, sem virar feed social;
- chamadas filtradas nao sao mais ocultadas por limite silencioso.
...
- publicacao;
- KPIs de operacao.
- segmento `Organizando` para jogador puro.

Status 2026-05-15:

- `COMP-UX-01` separou `Jogando`, `Organizando` e `Descobrir`;
- `QA-DESIGN-01` removeu badges `0` e ocultou `Organizando` da primeira leitura do jogador puro;
- usuario com competicoes organizadas preserva o modo de organizacao em contexto proprio.
...
- `PLAYER-UX-07` concluido;
- `/perfil` abre por finalidade, com tabs `Perfil`, `Historico`, `Preferencias` e `Conta`;
- dados basicos ficam em `Perfil`;
- partidas/competicoes do jogador ficam em `Historico`;
- XP, conquistas, leitura tecnica e post de partida ficam como detalhes secundarios, nao primeira dobra;
- lembretes ficam em `Preferencias`;
- suporte, privacidade, sair da conta e atalhos profissionais ficam em `Conta`;
- atalhos de organizador so aparecem quando ha competicoes organizadas pelo usuario.
