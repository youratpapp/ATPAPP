-- Fix tournament player result submission RPC return-column ambiguity
-- Date: 2026-05-20
--
-- The old RETURNS TABLE signature exposes output variables named like
-- tournament_id, class_key and match_index. In PL/pgSQL those names can
-- collide with unqualified column references inside ON CONFLICT and SELECTs.
-- Returning SETOF the table keeps the API columns identical for clients while
-- removing the conflicting output variables.

drop function if exists public.app_submit_tournament_match_result(uuid, text, text, text, text, integer, text, text, text);
...
returns setof public.tournament_match_result_submissions
language plpgsql
security definer
